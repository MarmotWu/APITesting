...
class Test_Center:
    ...
    def test_findUsers(self,CenterBuilder,resource,method):
        ...
        response = CenterBuilder.JsonRequest(method,resource,body)
        assert response["status_code"] == 200
        if response["code"] == 200 and len(response["data"]["result"]) != 0:
    ...
    def test_getUserAuthPDSPackage(self,CenterBuilder,method):
        ...
        if response["code"] == 200 and len(response["data"]) != 0:
            ...
            assert len(response["data"]) >= self.packageInfoNumber
        else:
            assert False
    ...
    def test_findUserDeptInfo(self,CenterBuilder,method):
        ...
        if response["code"] == 200 and len(response["data"]) != 0:
            #判断是否授权了顶级公司节点
            assert self.orgId in response["data"]["orgIds"]
        else:
            assert False
    ...
    def test_findUserProjectInfo(self,CenterBuilder,method):
        ...
        assert response["status_code"] == 200
        if response["code"] == 200 and len(response["data"]) != 0:
            assert response["data"]["isAll"] == True
        else:
            assert False
    ...
    def test_findUserDataDirectoryIds(self,CenterBuilder,method):
        ...
        assert response["status_code"] == 200
        if response["code"] == 200 and len(response["data"]) != 0:
            assert response["data"]["chooseAll"] == True
        else:
            assert False
